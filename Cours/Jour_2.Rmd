...
```
Uh oh, il y a les NA! is.na(x) dit si un variable est NA, et ! est 'not'm donc !is.na(x) dit si un variable n'est pas NA. 
Faites un data frame ou nombre_denfants n'est pas NA. 
```{r}
...
```
Faites un data frame ou le interviewer est simple ou participant mais pas adoptee. Est-ce que tu peux penser en deux manere de faire ca?
```{r}
